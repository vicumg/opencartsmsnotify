...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
        ...
        if (!empty($data['filter_product_name'])) {
             $sql .= " JOIN `oc_order_product` op ON o.order_id=op.order_id JOIN `oc_product_description` pd ON pd.product_id=op.product_id ";
         } 
        ...
        if (!empty($data['filter_product_name'])) {
             $sql .= "  and pd.name='" . $data['filter_product_name'] . "'";
        } 
		...
		if (!empty($data['filter_customer'])) {
			...
			$sql .= " AND CONCAT(o.firstname, ' ', o.lastname, ' ', o.telephone, ' ', o.novaposhta_cn_number, ' ', o.email, ' ', o.shipping_city, ' ', o.shipping_address_1, ' ', o.payment_method, ' ', o.shipping_method) LIKE '%" . $this->db->escape($data['filter_customer']) . "%'";
	...
	//vicumg
    public function   getOrdersByTabs($filter_data){
	    if ($filter_data['filter_order_status_id'] !==''){
	        return $this->getOrders($filter_data);
        }
        $this->load->model('localisation/order_status');
        $statuses =  $this->model_localisation_order_status->getOrderStatuses();
        $orders=[];
        foreach ($statuses as $status){
            $filter_data['filter_order_status_id'] = $status['order_status_id'];
            $newOrders=$this->getOrders($filter_data);
